...
```shell
# 将 Person 与 action 解耦，互相依赖，如果需要再次添加状态
# person  person.Accept( Accept accept ) -> accept.do(this)
#  通过 accept的实现来控制状态
# Accept   accept( Man man ) dosomething
# 很绕，但是有用
```
...
- **注意事项**
  ...
  4.  **缺点**
  5.  具体元素对访问者公布细节，也就是说访问者关注了其他类的内部细节，这是迪米特法则所不建议的, 这样造成了具体元素变更比较困难
  6.  违背了依赖倒转原则。访问者依赖的是具体元素，而不是抽象元素
  7. 因此，如果一个系统有比较稳定的数据结构，又有经常变化的功能需求，那么访问者模式就是比较合适的.



## 15）迭代器模式Iterator Pattern

![](image/Snipaste_2022-11-19_12-43-21.png)
...
## 16）观察者模式 Observer Mode

![](image/Snipaste_2022-11-20_13-26-21.png)
...
![](image/Snipaste_2022-11-20_14-27-41.png)
...
```sh
# 减少子系统之间的 交互，让 中介 去交互
# 子系统之间尽量不要交流，让中介来处理，不然会增加子系统之间的复杂度。
```
...
![](image/Snipaste_2022-11-20_18-24-58.png)
...
![](image/Snipaste_2022-11-21_09-51-31.png)

- **基本介绍**
  1. 在编译原理中，一个算术表达式通过词法分析器形成词法单元，而后这些词法单元再通过语法分析器构建语法分析树，最终形成一颗抽象的语法分析树。这里的词法分析器和语法分析器都可以看做是解释器
  2.  解释器模式（Interpreter Pattern）：是指**给定一个语言(表达式)**，定义它的文法的一种表示，并定义一个**解释器**，使用**该解释器来解释语言中的句子**(表达式)
...
public class Jdk {
    public static void main(String[] args) {
        ...
        Expression expression = spelExpressionParser.parseExpression("2*(9+6/3-5)+4");
...
- **基本介绍**
  1. 策略模式（Strategy Pattern）中，定义**算法族（策略组）**，分别封装起来，让他们之间可以互相替换，此模式让**算法的变化**独立于**使用算法的客**户
